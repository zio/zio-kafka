  ...
  suite("consumer test suite")(
    ...
    testM("commit offsets for all consumed messages"){
      ...
      for {
        ...
        fib <- consumeWithStrings("group3", "client3", subscription)({
            (key, value) =>
              (for {
                messagesSoFar <- messagesReceived.update(_ :+ (key -> value))
                _             <- Task.when(messagesSoFar.size == nrMessages)(done.succeed(()))
          ...
          .fork
        _ <- done.await *> Live.live(ZIO.sleep(3.seconds)) // TODO the sleep is necessary for the outstanding commits to be flushed. Maybe we can fix that another way
        _ <- fib.interrupt
        _ <- fib.join.ignore
    ...
    testM("fail when the consuming effect produces a failure") {
      ...
      for {
        ...
        consumeResult <-  consumeWithStrings("group3", "client3",subscription) { (key, value) =>
          ...
          .run
       } yield consumeResult.fold(_ => assertCompletes, _ => assert("result", equalTo("Expected consumeWith to fail")))
    }
