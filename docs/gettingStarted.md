...
This example does the same as the previous, except that messages are produced in a transaction. 
A transactional message is committed by committing the corresponding consumer offset. Consumers 
that use the isolation strategy "read-committed", will only read committed messages.

Again messages are consumed from topic `topic_a` and produced to `topic_b`. Also here processing is 
done in chunks for more efficiency.
