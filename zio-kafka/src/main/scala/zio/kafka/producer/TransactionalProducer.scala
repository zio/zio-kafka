...
object TransactionalProducer {
  ...
  final case class TransactionLeaked(offsetBatch: TransactionalOffsetBatch) extends Throwable
  ...
  private final case class LiveTransactionalProducer(
    ...
    private def commitTransactionWithOffsets(offsetBatch: TransactionalOffsetBatch): Task[Unit] =
      (
        for {
          _ <- ZIO.attemptBlocking {
                 val offsetAndMetadata =
                   offsetBatch.offsetBatch.map { case (topicPartition, offset) =>
                     topicPartition -> new OffsetAndMetadata(offset.offset + 1)
                   }.asJava
    ...
    def createTransaction: ZIO[Scope, Throwable, Transaction] =
      semaphore.withPermitScoped *> {
        ZIO.acquireReleaseExit {
          for {
            offsetBatchRef <- Ref.make(TransactionalOffsetBatch.empty)
