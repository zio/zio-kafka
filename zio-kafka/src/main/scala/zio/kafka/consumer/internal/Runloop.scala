...
private[consumer] final class Runloop(
   ...
   */
  private def handleShutdown(cmds: Chunk[Command]): Task[Unit] =
    ZIO
      .foreachDiscard(cmds) {
        case req: Request =>
          req.end
        ...
        case _ @Command.Commit(_, cont) =>
          cont.fail(new KafkaException("Consumer is shutting down")) // TODO can we just allow it?
      }
  ...
  def run: ZIO[Scope, Nothing, Fiber.Runtime[Throwable, Any]] = {
    def processCommands(state: State, wait: Boolean): Task[State] =
      for {
        ...
        updatedState <- if (isShutdown) handleShutdown(commands).as(state)
                        else ZIO.foldLeft(commands)(state)(handleOperational)
      } yield updatedState
